...
async function isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, lang, threadsData) {
	...
	// ==========    Check Thread    ========== //
	if (isGroup == true) {
		// check if thread suspended (custom suspension system) - use live database
		let liveThreadData;
		let isSuspended = false;
		try {
			liveThreadData = await threadsData.get(threadID);
			isSuspended = liveThreadData.data?.suspended === true;
		} catch (error) {
			console.error(`[SUSPENSION DEBUG] Error getting thread data for ${threadID}:`, error);
			// Fallback to cached data if database query fails
			liveThreadData = threadData;
			isSuspended = threadData.data?.suspended === true;
		}
		console.log(`[SUSPENSION DEBUG] ThreadID: ${threadID}, CachedSuspended: ${threadData.suspended}, LiveSuspended: ${liveThreadData.data?.suspended}, CommandName: ${commandName}, SenderID: ${senderID}, IsAdmin: ${adminBot.includes(senderID)}, MessageBody: ${message.body?.substring(0, 50)}...`);
		console.log(`[SUSPENSION DEBUG] Full liveThreadData.data:`, JSON.stringify(liveThreadData.data, null, 2));
		
		if (isSuspended && !adminBot.includes(senderID)) {
			console.log(`[SUSPENSION DEBUG] Thread is suspended, checking command: ${commandName}`);
			// Only allow supportgc command and its aliases for non-admins
			const allowedCommands = ['supportgc', 'support', 'maingroup', 'helpgroup'];
			if (!allowedCommands.includes(commandName)) {
				console.log(`[SUSPENSION DEBUG] Blocking command: ${commandName} in suspended thread: ${threadID}`);
				
				// Simplified deduplication - use threadID + senderID + hour to prevent spam
				const currentHour = Math.floor(Date.now() / (1000 * 60 * 60)); // Current hour timestamp
				const suspensionKey = `${threadID}_${senderID}_${currentHour}`;
				if (!global.suspensionMessageSent) global.suspensionMessageSent = new Set();
				
				// Check if we've already sent a suspension message for this user in this thread in this hour
				if (!global.suspensionMessageSent.has(suspensionKey)) {
					global.suspensionMessageSent.add(suspensionKey);
					console.log(`[SUSPENSION DEBUG] Sending suspension message for key: ${suspensionKey}`);
					
					// Clean up old entries to prevent memory leaks (keep last 100 entries)
					if (global.suspensionMessageSent.size > 100) {
						const entries = Array.from(global.suspensionMessageSent);
						global.suspensionMessageSent = new Set(entries.slice(-50));
					}
					
					const suspensionMessage = `🚫 𝗔𝗖𝗖𝗘𝗦𝗦 𝗗𝗘𝗡𝗜𝗘𝗗\n━━━━━━━━━━━━━━━\n\n⚠️ আপনার group এ Bot ব্যবহারের জন্য অনুমদিত নয়!\n\n📋 𝗦𝗨𝗣𝗣𝗢𝗥𝗧 এর জন্য:\n• ${global.utils.getPrefix(threadID)}supportgc - Main group এ join করুন\n• Support Link: https://m.me/j/AbaOx18fo5rF5IS2/\n• Bot Admin এর সাথে যোগাযোগ করুন\n\n🌸 ধন্যবাদ আপনার সহয়োগিতার জন্য! 💖`;
					try {
						await message.reply(suspensionMessage);
						console.log(`[SUSPENSION DEBUG] ✅ Suspension message sent successfully to thread: ${threadID}`);
					} catch (error) {
						console.error(`[SUSPENSION DEBUG] ❌ Failed to send suspension message:`, error);
					}
				} else {
					console.log(`[SUSPENSION DEBUG] Skipping duplicate suspension message for key: ${suspensionKey}`);
				}
				return true;
			}
		}
	...
	return async function (event, message) {
		...
		*/
		...
		async function onStart() {
			...
			if (await isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, langCode, threadsData))
				return;
			if (!command)

...
module.exports = async function (databaseType, threadModel, api, fakeGraphql) {
	...
	switch (databaseType) {
		case "mongodb": {
			// Memory optimization: Load threads with pagination and lean queries
			console.log('📊 [MEMORY] Loading threads with memory optimization...');
			
			// Check if we have optimized query function
			if (global.optimizedMongoQuery) {
				Threads = (await global.optimizedMongoQuery(threadModel, {}, { 
					limit: 500, // Limit to 500 threads max
					select: '-_id -__v' // Exclude MongoDB internal fields
				}));
			} else {
				// Fallback to original method with limits
				Threads = (await threadModel.find({}, '-_id -__v')
					.lean()
					.limit(500)
					.sort({ updatedAt: -1 }));
			}
			
			console.log(`📊 [MEMORY] Loaded ${Threads.length} threads`);
			break;
		}
		case "sqlite": {

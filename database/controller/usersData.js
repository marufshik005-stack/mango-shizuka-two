...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
	...
	switch (databaseType) {
		case "mongodb": {
			// Memory optimization: Load users with pagination and lean queries
			console.log('📊 [MEMORY] Loading users with memory optimization...');
			
			// Check if we have optimized query function
			if (global.optimizedMongoQuery) {
				Users = (await global.optimizedMongoQuery(userModel, {}, { 
					limit: 1000, // Limit to 1000 users max
					select: '-_id -__v' // Exclude MongoDB internal fields
				}));
			} else {
				// Fallback to original method with limits
				Users = (await userModel.find({}, '-_id -__v')
					.lean()
					.limit(1000)
					.sort({ updatedAt: -1 }));
			}
			
			console.log(`📊 [MEMORY] Loaded ${Users.length} users`);
			break;
		}
		case "sqlite": {

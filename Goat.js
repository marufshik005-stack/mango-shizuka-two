...
// Load memory optimizer FIRST to prevent OOM issues
try {
    require('./memory-optimizer.js');
    console.log('✅ [GOAT.JS] Memory optimizer loaded successfully');
} catch (error) {
    console.log('⚠️ [GOAT.JS] Could not load memory optimizer:', error.message);
}

// Load emergency fix for ws3-fca stack overflow IMMEDIATELY
try {
    require('./fix-ws3-fca-stack-overflow.js');
    console.log('✅ [GOAT.JS] Emergency stack overflow fix loaded successfully');
} catch (error) {
    console.log('⚠️ [GOAT.JS] Could not load emergency fix:', error.message);
}

process.on('unhandledRejection', error => console.log(error));
process.on('uncaughtException', error => {
    // Check for the specific stack overflow we're trying to fix
    if (error.message && error.message.includes('Maximum call stack size exceeded')) {
        if (error.stack && error.stack.includes('resolveAttachmentUrl')) {
            console.log('🚨 [GOAT.JS] Caught ws3-fca resolveAttachmentUrl stack overflow!');
            console.log('🔄 [GOAT.JS] Bot will restart to recover...');
...
// Only use .dev files when explicitly in development mode, not in production
const useDevFiles = NODE_ENV === 'development';
const dirConfig = path.normalize(`${__dirname}/config${useDevFiles ? '.dev.json' : '.json'}`);
const dirConfigCommands = path.normalize(`${__dirname}/configCommands${useDevFiles ? '.dev.json' : '.json'}`);
const dirAccount = path.normalize(`${__dirname}/account${useDevFiles ? '.dev.txt' : '.txt'}`);
...
// Load cookie persistence patch to prevent dashboard cookie clearing
require('./cookie-fix-patch.js');
...
// ———————————————— BOT STATS SAVING ———————————————— //
// Function to save bot statistics to file for dashboard communication
function saveBotStatsToFile() {
    try {
        const statsData = {
            totalThreads: global.db?.allThreadData?.length || 0,
            totalUsers: global.db?.allUserData?.length || 0,
        ...
        // Don't log errors here as it might spam console
        // console.log('Could not save bot stats:', error.message);
    }
}

// Save stats every 30 seconds
setInterval(saveBotStatsToFile, 30000);

// Save stats immediately when bot data changes
process.on('botDataUpdated', saveBotStatsToFile);
